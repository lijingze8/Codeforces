...
void init() {
    int L1, L2, L3, L4;
    L1 = n + n + m + m, L2 = n + n * m, L3 = 2 * n + m, L4 = n + m - 1;
    for(int i = 1; i <= n * m; i++) scanf("%d", &a[i + L1].v);
    for(int i = L1 + 1; i <= L1 + n * m; i++)
        a[i].L = i - 1, a[i].R = i + 1, a[i].U = i - m, a[i]. D = i + m; 
    for(int i = L1 + 1; i <= L1 + m; i++)
        a[i - L1].D = i, a[i].U = i - L1;
    for(int i = L1 + n * m - m + 1; i <= L1 + n * m; i++)
        a[i - L2].U = i, a[i].D = i - L2;
    for(int i = L1 + 1; i <= L1 + n * m; i += m)
        a[i - L3].R = i, a[i].L = i - L3, L3 += m - 1;
    for(int i = L1 + m; i <= L1 + n * m; i += m)
        a[i - L4].L = i, a[i].R = i - L4, L4 += m - 1;
}

void outit() {
    for(int i = m + 1; i <= n + m; i++) {
        int nw = a[i].R;
        while (nw != i + n + m) printf("%d ", a[nw].v), nw = a[nw].R;
        printf("\n");
    }
}
...
void work() {
    ...
    p1 = Down(y1, x1), p2 = Down(y2, x2), s = w;
    while (s--) {
        t1 = a[p1].U, t2 = a[p2].U;
        a[p1].U = t2, a[p2].U = t1;
        a[t1].D = p2, a[t2].D = p1;
